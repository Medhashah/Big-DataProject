...
# Usage: spark-submit --conf spark.driver.memory=5G --conf spark.executor.memoryOverhead=5G --conf spark.rpc.message.maxSize=1024 --conf spark.driver.maxResultSize=3G --conf spark.pyspark.python=$PYSPARK_PYTHON --executor-memory 5G task1.py [ list_file_containing_dataset_paths | dataset_paths ] <output_filename>
...
def process_dataset(filename):
    ...
    for column_name in column_list:
        ...
        for j in col_dict.keys():
            ...
            if j == "REAL":
                dt_dict["max_value"] = float(rdd.max())
...
for dataset_name in dataset_names:
    ...
    if (count_processed_files == chunk_limit):
        count_processed_files = 0
